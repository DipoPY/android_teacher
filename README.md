...
> Статус: готовы каркас сборки, доменная логика (прохождение тем, повторения FSRS, микс, серия, XP, достижения, таймер), хранение прогресса, компилятор контента и стартовый контент (2 раздела, 5 тем, 59 вопросов). Следующий шаг — UI на Compose (этапы 1, 3–7 из раздела [«Как делать»](#как-делать)).
...
| Ответ в уроке | Оценка для планировщика |
|---|---|
| Была хотя бы одна ошибка (даже если потом ответил верно) | `Again` — вопрос вернётся в повторение завтра |
| Верно с первой попытки | `Good` — первый повтор через ~3 дня, дальше интервалы растут |
...
│   ├── platform/                    # KeyValueStore (localStorage), AudioRecorder (MediaRecorder), Today
...
Интерфейс оставляет возможность заменить алгоритм (например, простой Leitner для отладки) без изменений в домене.
Отличие от эталонного FSRS: после `Again` вопрос всегда возвращается завтра, чтобы доучить его, пока свежо.
...
```
content/
├── sections.yaml                       # sections: [kotlin, coroutines] — порядок разделов
└── kotlin/                             # id раздела = имя папки
    ├── section.yaml                    # title + topics: порядок тем (порядок открытия)
    └── data-class/                     # id темы = kotlin.data-class
        ├── topic.yaml                  # title
        ├── quiz/                       # вопросы квиза, порядок — по имени файла
        │   ├── q01.md
        │   └── q02.md
        └── open/                       # открытые вопросы для таймера
            └── o01.md
```

`id` вопроса задаётся в шапке и **не меняется никогда**: к нему привязан прогресс учеников. Файл можно переименовать — прогресс сохранится.
...
Типы: `single` (ровно один `- [x]`), `multiple` (один или больше `- [x]`), `true-false` — вместо `## Options` раздел `## Answer` со значением `true` или `false`.
В формулировке допускается один блок кода — он показывается отдельно, с подсветкой.
...
- проверяет обязательные поля, формат и уникальность `id`, в `single` ровно один `[x]`, в `multiple` ≥ 1, у открытого вопроса ровно 3 подсказки и ≥ 3 пункта чек-листа, в теме 10–15 вопросов квиза, все папки перечислены в `section.yaml` / `sections.yaml`;
- собирает всё в `tools/content-compiler/build/content/content.json`, откуда его забирает приложение;
- показывает сразу все ошибки: `content/kotlin/data-class/quiz/q02.md: в single ровно один верный вариант`.
...
```bash
./gradlew build                                          # все тесты, проверка контента, сборка
./gradlew :tools:content-compiler:compileContent         # проверить и собрать контент
./gradlew :core:domain:jvmTest                           # тесты одного модуля (логика тестируется на JVM)
./gradlew :app:web:wasmJsBrowserDevelopmentRun           # dev-сервер (после появления app/web)
./gradlew :app:web:wasmJsBrowserDistribution             # продакшен-сборка (после появления app/web)
```

Требования: JDK 17+, актуальный Chrome/Firefox. Для записи микрофона сайт должен открываться по `https` или `localhost`.
Compose Multiplatform тянет артефакты `androidx.*` из Google Maven — нужен доступ к `dl.google.com`.

Правила для разработки вместе с Claude — в [`CLAUDE.md`](CLAUDE.md).
